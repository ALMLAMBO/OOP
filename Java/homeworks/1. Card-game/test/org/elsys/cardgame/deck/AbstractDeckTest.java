...
public abstract class AbstractDeckTest {
	...
	public void testDeal() {
		Hand hand = deck.deal();
		assertEquals("Hand size is correct"
				, handSize(), hand.size());

		assertEquals("New size of deck is correct"
				, deckSize() - handSize(), deck.size());

		clearDeck.getCards().removeAll(hand.getCards());
		assertEquals("Cards left in deck are correct"
				, clearDeck.getCards(), deck.getCards());

		hand.getCards().addAll(deck.getCards());
		assertEquals("Cards in hand are correct"
				, defaultDeck().getCards(), hand.getCards());
	}

	@Test
	public void testShuffle() {
		deck.shuffle();
		assertNotEquals("Shuffle works correctly"
				, clearDeck.getCards(), deck.getCards());
	}
	...
	public void testSort() {
		deck.shuffle();
		deck.sort();
		assertEquals("Sort works correctly"
				, clearDeck.getCards(), deck.getCards());
	}
	...
	public void testDrawTopCard() {
		...
		assertEquals("Deck size is correct"
				, deckSize() - 1, deck.size());
		assertEquals("Card rank is correct"
				, orderedRanks().get(0), card.getRank());
		assertEquals("Card suit is correct"
				, Suit.CLUBS, card.getSuit());
	}
	...
	public void testDrawBottomCard() {
		...
		assertEquals("Card is correct"
				, ranks.get(ranks.size() - 1), card.getRank());
		assertEquals("Card suit is correct"
				, Suit.SPADES, card.getSuit());
	}
